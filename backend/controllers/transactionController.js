...
exports.buyStock = async (req, res) => {
  ...
  try {
    ...
    if (priceResult.length === 0) {
      ...
      // 사용자의 현재 보유 현금 확인
      const [assetResult] = await db.query('SELECT cash FROM assets WHERE user_id = ?', [user_id]);
      if (assetResult.length === 0 || assetResult[0].cash < totalPrice) {
        return res.status(400).json({ error: '잔액이 부족합니다' });
      }
  
      // 현금 차감
      await db.query('UPDATE assets SET cash = cash - ? WHERE user_id = ?', [totalPrice, user_id]);
      ...
      if (existing.length === 0) {
        ...
        const newAvg = Math.round((prevQty * prevAvg + quantity * price) / newQty);
      ...
      // 거래 기록
      await db.query(
        'INSERT INTO transactions (type, user_id, stock_id, quantity, total_price, created_at) VALUES (?, ?, ?, ?, ?, NOW())',
        ['buy', user_id, stock_id, quantity, totalPrice]
      );
  ...
  exports.sellStock = async (req, res) => {
    ...
    try {
      ...
      // 현금 증가
      await db.query(
        'UPDATE assets SET cash = cash + ? WHERE user_id = ?',
        [totalPrice, user_id]
      );
      ...
      await db.query(
        'INSERT INTO transactions (type, user_id, stock_id, quantity, total_price, created_at) VALUES (?, ?, ?, ?, ?, NOW())',
        ['sell', user_id, stock_id, quantity, totalPrice]
      );
