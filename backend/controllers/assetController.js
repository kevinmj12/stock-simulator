...
exports.getMyAssets = async (req, res) => {
  ...
  try {
    // 현금 조회
    const [cashResult] = await db.query('SELECT cash FROM assets WHERE user_id = ?', [user_id]);
...
exports.updateCash = async (req, res) => {
  ...
  try {
    const [existing] = await db.query('SELECT id FROM assets WHERE user_id = ?', [user_id]);

    if (existing.length === 0) {
      // 새로 추가
      await db.query('INSERT INTO assets (user_id, cash, created_at) VALUES (?, ?, NOW())', [user_id, cash]);
    } else {
      // 기존 현금 업데이트
